...
static void createLogger() {
  ...
  spdlog::sink_ptr pFileSink = std::make_shared<spdlog::sinks::basic_file_sink_mt>("logs/stereo_euroc.txt", true);
...
int main(int argc, char **argv) {
  if(argc != 6) {
    cerr << endl
         << "Usage: ./stereo_euroc path_to_vocabulary path_to_settings path_to_left_folder path_to_right_folder path_to_times_file"
         << endl;
    return 1;
  }
  ...
  LoadImages(std::string(argv[3]), std::string(argv[4]), std::string(argv[5]), vstrImageLeft, vstrImageRight, vTimeStamp);

  if(vstrImageLeft.empty() || vstrImageRight.empty()) {
    spdlog::error("ERROR: No images in provided path.");
    return EXIT_FAILURE;
  }

  if(vstrImageLeft.size() != vstrImageRight.size()) {
    spdlog::error("ERROR: Different number of left and right images.");
    return EXIT_FAILURE;
  }
  ...
  if(K_l.empty() || K_r.empty() || P_l.empty() || P_r.empty() || R_l.empty() || R_r.empty() || D_l.empty() || D_r.empty() ||
    ...
    spdlog::error("ERROR: Calibration parameters to rectify stereo are missing!");
    return EXIT_FAILURE;
  }
  ...
  for(int ni = 0; ni < nImages; ni++) {
    ...
    if(imLeft.empty()) {
      spdlog::debug("Failed to load image at: {}", std::string(vstrImageLeft[ni]));
      return 1;
    }

    if(imRight.empty()) {
      spdlog::debug("Failed to load image at: {}", std::string(vstrImageRight[ni]));
      return 1;
    }
